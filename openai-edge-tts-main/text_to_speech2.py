...
def create_book_specific_output_dir(input_file, base_output_dir):
    """Create a book-specific output directory based on the input filename."""
    # Get base filename without extension
    base_name = os.path.splitext(os.path.basename(input_file))[0]
    
    # Create book-specific directory
    book_output_dir = os.path.join(base_output_dir, f"{base_name}_mp3s")
...
def process_text_file(input_file, output_dir=OUTPUT_DIR, voice=VOICE, auto_create_subdir=True):
    """Process an entire text file into multiple audio files."""
    # If auto_create_subdir is True, create a book-specific subdirectory
    if auto_create_subdir:
        output_dir = create_book_specific_output_dir(input_file, output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-subdir", action="store_true", help="Don't create book-specific subdirectory")
    ...
    print(f"Auto-create subdirectory: {not args.no_subdir}\n")
    ...
    if not args.test or os.path.exists(args.input_file):
        ...
        if not os.path.exists(args.input_file):
            ...
            process_text_file(args.input_file, args.output_dir, args.voice, not args.no_subdir)